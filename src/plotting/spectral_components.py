...
def plot_spectral_components(spectrum_path='results/spectrum.csv',
    ...
    for j in range(len(time_steps)):
      plt.plot(time_steps[j:j+2]/len(time_steps)*tf, frequencies[j:j+2, 2] * sqrt_d[j:j+2], lw=lw, color=viridis(powers[j, 2]/max_power))
    ...
    for i in range(tot_frequencies)[::-1]:
        ...
        plt.plot(time_steps/len(time_steps)*tf, powers[:, i]*sqrt_d, label=fr'$i = {tot_frequencies - i - 1}$', lw=lw, color=viridis(tot_frequencies- i- 1))
